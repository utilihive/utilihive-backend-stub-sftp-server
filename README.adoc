...
The SFTP Server Stub supports both absolute and relative paths. Paths without `/`
prefix are interpreted relative to `defaultDirectory` (`/`, unless overridden
when initializing the context).

In connection to flow testing you should keep in mind that the `readFiles` and
`writeFiles` processors in Utilihive Flow Server interpret properties `path` as
relative to default directory (typically user's home directory) whether they
are prefixed with `/` or not. `moveToFolder` and `errorFolder` are interpreted
as relative to `path` property if prefixed with `/` and relative to default
directory if not prefixed with `/`.
