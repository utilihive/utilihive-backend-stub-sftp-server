...
The SFTP Server Stub supports both absolute (`/path/to/file`) and relative (`path/to/file`) paths.
Quite commonly SFTP servers present client with a limited view of the filesystem, where root of what's available to the user is presented as `/` and that is the initial directory where user starts after login.
...
If that's not your case and your server presents you with let's say `/home/myuser` after logging in, then you should set the path when initializing the context.
----
withSftpServer(initialDirectory = "/home/myuser") {
  // ...
}
----
Paths without `/` prefix will be interpreted relative to given `initialDirectory`, so in the example above, `/files/in` will point to `/files/in` and `files/in` point to `/home/myuser/files/in`.
In connection to flow testing you should keep in mind that the `readFiles` and
`writeFiles` processors in Utilihive Flow Server always interpret property `path` as relative to that directory.
